...
import { mockArchiveTabs } from '../../mock/data';
...
const handleWriteProspectFollowUp = () => {
  // 检查是否有潜客数据（潜客档案Tab）
  const hasProspectData = mockArchiveTabs.some(tab => tab.id === 'prospect');
  if (!hasProspectData) {
    toastMessage.value = '暂无潜客数据，请先添加潜客信息';
    showToast.value = true;
    setTimeout(() => { showToast.value = false; }, 2000);
    return;
  }
  // 有潜客数据，跳转到跟进填写页面
  router.push('/features/add-remark');
};
  ...
  <div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 pt-3 pb-8 flex flex-col gap-3 z-40">
      <div class="flex justify-center w-full">
        <button 
          @click="handleWriteProspectFollowUp"
          class="relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-[15px] font-semibold shadow-lg overflow-hidden isolate transition-all duration-300 active:scale-[0.97] w-full max-w-[220px] justify-center"
          style="background: linear-gradient(135deg, #2563eb 0%, #4f46e5 50%, #7c3aed 100%); color: white;"
        >
          <!-- 光晕效果 -->
          <span class="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0 -translate-x-full animate-[shimmer_2.5s_ease-in-out_infinite] z-0"></span>
          <!-- 边缘发光 -->
          <span class="absolute inset-0 rounded-xl ring-2 ring-blue-400/30 z-0"></span>
          <span class="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-blue-400/20 via-indigo-400/20 to-violet-400/20 blur-sm z-[-1]"></span>
          <PenSquare :size="18" :strokeWidth="1.8" class="relative z-10 drop-shadow-sm" />
          <span class="relative z-10">写潜客跟进</span>
        </button>
      </div>
